...
import { getCookiePreferences, setCookiePreferences as setCookiePrefs } from '@/utils/cookieManager';
...
const CookieConsent = () => {
  ...
  const handleSavePreferences = () => {
    ...
    try {
      setCookiePrefs(normalizedPreferences);
    } catch {
      try {
        localStorage.setItem('cookie-preferences', JSON.stringify(normalizedPreferences));
        window.dispatchEvent(new CustomEvent('cookie-preferences-changed', { detail: normalizedPreferences }));
      } catch {
        // ignore
      }
    }
  ...
  const modalContent = showPopup ? (
    <div className="fixed inset-0 z-[9999] pointer-events-none">
      <div className="w-[86vw] max-w-md pointer-events-auto bg-card text-card-foreground rounded-2xl shadow-2xl border border-border transition-transform transform-gpu fixed right-3 bottom-3 sm:right-6 sm:bottom-6 max-h-[90svh] overflow-hidden">
        <div className="flex max-h-[90svh] flex-col min-h-0">
          <div className="flex-1 overflow-y-auto px-3 sm:px-4 pt-1 pb-3">
            ...
            {!showSettings ? (
              <div className="mt-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
                  <Button onClick={handleAcceptAll} className="flex-1 py-2 text-sm px-3 bg-slate-900 text-white font-semibold hover:bg-slate-800">{t('acceptAll')}</Button>
                  <Button onClick={() => setShowSettings(true)} className="flex-1 py-2 text-sm px-3 bg-slate-100 text-slate-900 border border-slate-200 hover:bg-slate-200">{t('customize')}</Button>
                </div>
              </div>
            ) : (
              <div className="mt-4 space-y-4">
                ...
                {[
                  {
                    key: 'essentialFunctional',
                    title: `${t('necessary')}`,
                    description: `${t('necessaryDescription')} ${t('functionalDescription')}`,
                ...
                ].map(({ key, icon, enabled, toggle, title, description }) => (
                  <div key={key} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between p-3 bg-card border border-border rounded-lg">
                    ...
                    <div className="self-end sm:self-auto">
                      {toggle ? (
                        ...
                        <Slider checked={true} onChange={() => {}} className="pointer-events-none opacity-70" />
                      )}
                    </div>
                  </div>
                ))}
          ...
          {showSettings && (
            <div className="border-t border-border bg-card/95 backdrop-blur px-3 sm:px-4 py-3">
              <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
                <Button onClick={() => setShowSettings(false)} className="flex-1 py-2 text-sm px-3 bg-slate-100 text-slate-900 border border-slate-200 hover:bg-slate-200">{t('back')}</Button>
                <Button onClick={handleSavePreferences} className="flex-1 py-2 text-sm px-3 bg-slate-900 text-white hover:bg-slate-800">{t('save')}</Button>
              </div>
            </div>
          )}
        </div>
